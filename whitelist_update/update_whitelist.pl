...
my $dbh_modencode = DBI->connect("dbi:Pg:dbname=modencode_chado;host=heartbroken.lbl.gov", "db_public", "pw", { AutoCommit => 0 }) or die "Couldn't get modENCODE database $!";
my $dbh_wiki = DBI->connect("dbi:mysql:database=modencode_wiki;host=localhost", "user", "pw") or die "Couldn't get Wiki database $!";
...
my $sth = $dbh_modencode->prepare("SELECT x_dbxref.accession FROM x_dbxref 
  ...
  WHERE x_db.description = 'URL_mediawiki_expansion'
